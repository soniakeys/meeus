...
func ExampleApproxTimes() {
	...
	p := globe.Coord{
		Lon: common.NewAngle(false, 71, 5, 0).Rad(),
		Lat: common.NewAngle(false, 42, 20, 0).Rad(),
	}
	// Meeus gives us the value of 11h 50m 58.1s but we have a package
	// function for this:
	Th0 := sidereal.Apparent0UT(jd)
	α := common.NewRA(2, 46, 55.51).Rad()
	δ := common.NewAngle(false, 18, 26, 27.3).Rad()
...
func ExampleTimes() {
	...
	Th0 := sidereal.Apparent0UT(jd)
	α3 := []float64{
		common.NewRA(2, 42, 43.25).Rad(),
		common.NewRA(2, 46, 55.51).Rad(),
		common.NewRA(2, 51, 07.69).Rad(),
	}
	δ3 := []float64{
		common.NewAngle(false, 18, 02, 51.4).Rad(),
		common.NewAngle(false, 18, 26, 27.3).Rad(),
		common.NewAngle(false, 18, 49, 38.7).Rad(),
	}
	h0 := rise.Stdh0Stellar
	...
	fmt.Println("rising: ", common.NewFmtTime(rise))
	fmt.Println("transit:", common.NewFmtTime(transit))
	fmt.Println("seting: ", common.NewFmtTime(set))
	// Output:
	// rising:  12ʰ26ᵐ9ˢ
	// transit: 19ʰ40ᵐ30ˢ
	// seting:  2ʰ54ᵐ26ˢ
