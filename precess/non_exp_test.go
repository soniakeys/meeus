...
func TestEqProperMotionToEcl(t *testing.T) {
	...
	mλ, mβ := eqProperMotionToEcl(
		...
		meeus.NewAngle(false, 0, 0, 0.006).Rad(),
		2000.0,
		// eq coordinates from p. 132.
		new(coord.Ecliptic).EqToEcl(&coord.Equatorial{
			RA:  meeus.NewRA(10, 8, 22.3).Rad(),
			Dec: meeus.NewAngle(false, 11, 58, 2).Rad(),
		}, sε, cε))
	d := math.Abs((mλ - meeus.NewAngle(true, 0, 0, .2348).Rad()) / mλ)
	if d*169 > 1 { // 169 = significant digits of given lon
		t.Fatal("mλ")
	}
	d = math.Abs((mβ - meeus.NewAngle(true, 0, 0, 0.0813).Rad()) / mβ)
