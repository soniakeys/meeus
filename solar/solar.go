...
//	ν = true anomaly
func True(T float64) (s, ν base.Angle) {
	// (25.2) p. 163
	L0 := base.AngleFromDeg(base.Horner(T, 280.46646, 36000.76983, 0.0003032))
	M := MeanAnomaly(T)
	C := base.AngleFromDeg(base.Horner(T, 1.914602, -0.004817, -.000014)*
		math.Sin(M.Rad()) +
		(0.019993-.000101*T)*math.Sin(2*M.Rad()) +
...
func MeanAnomaly(T float64) base.Angle {
	// (25.3) p. 163
	return base.AngleFromDeg(base.Horner(T, 357.52911, 35999.05029, -0.0001537))
}
...
func Radius(T float64) float64 {
	...
	e := Eccentricity(T)
	// (25.5) p. 164
	return 1.000001018 * (1 - e*e) / (1 + e*math.Cos(ν.Rad()))
}
...
func ApparentLongitude(T float64) base.Angle {
	Ω := node(T)
	s, _ := True(T)
	return s - base.AngleFromDeg(.00569) -
		base.AngleFromDeg(.00478).Mul(math.Sin(Ω.Rad()))
}

func node(T float64) base.Angle {
	return base.AngleFromDeg(125.04 - 1934.136*T)
}
...
//	s = true geometric longitude, ☉
//	ν = true anomaly
func True2000(T float64) (s, ν base.Angle) {
	s, ν = True(T)
	s -= base.AngleFromDeg(.01397).Mul(T * 100)
	return
}

// TrueEquatorial returns the true geometric position of the Sun as equatorial coordinates.
func TrueEquatorial(jde float64) (α base.RA, δ base.Angle) {
	s, _ := True(base.J2000Century(jde))
	ε := nutation.MeanObliquity(jde)
	ss, cs := math.Sincos(s.Rad())
	sε, cε := math.Sincos(ε.Rad())
	// (25.6, 25.7) p. 165
	α = base.RAFromRad(math.Atan2(cε*ss, cs))
	δ = base.Angle(math.Asin(sε * ss))
	return
}
...
func ApparentEquatorial(jde float64) (α base.RA, δ base.Angle) {
	T := base.J2000Century(jde)
	λ := ApparentLongitude(T)
	ε := nutation.MeanObliquity(jde)
	sλ, cλ := math.Sincos(λ.Rad())
	// (25.8) p. 165
	ε += base.AngleFromDeg(.00256).Mul(math.Cos(node(T).Rad()))
	sε, cε := math.Sincos(ε.Rad())
	α = base.RAFromRad(math.Atan2(cε*sλ, cλ))
	δ = base.Angle(math.Asin(sε * sλ))
	return
}
...
//	β: ecliptic latitude
//	R: range in AU
func TrueVSOP87(e *pp.V87Planet, jde float64) (s, β base.Angle, R float64) {
	...
	Δβ := base.AngleFromSec(.03916).Mul(cλp - sλp)
	// (25.9) p. 166
	s -= base.AngleFromSec(.09033)
...
//  β: ecliptic latitude
//  R: range in AU
func ApparentVSOP87(e *pp.V87Planet, jde float64) (λ, β base.Angle, R float64) {
...
//	δ: declination
//	R: range in AU
func ApparentEquatorialVSOP87(e *pp.V87Planet, jde float64) (α base.RA, δ base.Angle, R float64) {
...
func aberration(R float64) base.Angle {
	// (25.10) p. 167
	return base.AngleFromSec(-20.4898).Div(R)
}
